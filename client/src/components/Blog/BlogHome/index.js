...
const BlogHome = (): React$Element<any> => {
  const { blogPosts, loading } = useSelector(
    (state: ReduxType): BlogStateType => state.blog,
  );
    ...
    <ul>
      {
        ...
        blogPosts.map((post: BlogEntryType): React$Element<any> => (
          <BlogEntry key={post._id} post={post} />
        ))
      }
    </ul>
  );
